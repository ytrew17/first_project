...
void show_playmap(struct user* user)
{
    ...
    printf("Score: %d, Life: %d, Reset: %d", user->get_point, user->life, user->reset);
}
...
void user_input(struct user* user)
{
    ...
    else if ((c1 == 'r') && (user->reset > 0))
    {

        user->dx = user->dy = 0;
        user->reset -= 1;
        user->target = 'A'+NUM_ITEM+3;

    }
    else user->dx =0, user->dy = 0;
}
...
void move_user(struct user* user)
{
    if (map[user->cur_y + user->dy][user->cur_x + user->dx] != 'o')
    {
        ...
        else if ('A' - 1 < map[user->cur_y][user->cur_x] < 'A' + NUM_ITEM + 1)
        {
...
int check_for_end(struct user* user)
{
    if ((user->life == 0)|| (user->get_point == 15)) return 0;
